## Реализация `withDebounce`
```ts
const debouncedSearchResource = engine.resource(
  withDebounce(
    ...
    // Запрос отправится строго через 500 мс после того, как пользователь закончит ввод
    { delay: 500 }
  ),
  searchSignal
);
...
### Практические кейсы применения декоратора `withDebounce`
