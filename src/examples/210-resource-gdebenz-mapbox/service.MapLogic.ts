...
export class MapLogic extends BaseREService {
  ...
  // НАКОПИТЕЛЬНЫЙ КЭШ: Храним GeoJSON фичи заправок по их ID
  private geojsonFeaturesCache = new Map<number, any>()
  ...
  public initializeMap = async (container: HTMLDivElement) => {
    ...
    try {
      ...
      console.error('Ошибка初始化 Mapbox:', e)
    }
  }

  /**
   * Инициализирует реактивные эффекты стейт-менеджера для синхронизации данных и камеры.
   *
   * @private
   * @returns {void}
   */
  private initReactiveEffects() {
    ...
    /**
     * РЕАКТИВНЫЙ ЭФФЕКТ СИНХРОНИЗАЦИИ МАРКЕРОВ
     *
     * Зачем нужен этот эффект:
     * 1. Декларативное связывание: Метод `this.engine.effect` автоматически подписывается
     *    на изменения реактивного ресурса `stationsResource.data`. Как только бэкенд возвращает
     *    новый массив АЗС для текущей области видимости, эффект перезапускается.
     * 2. Оптимизация WebGL (Diffing): Mapbox рендерит объекты через видеокарту на уровне единого
     *    слоя данных (`Source`). Метод `renderMarkers` внутри эффекта накапливает уникальные фичи
     *    в постоянный кэш `geojsonFeaturesCache` и точечно отправляет изменения через `source.setData()`.
     * 3. Защита от UX-багов: Благодаря реактивной изоляции, точки на карте не перерисовываются
     *    с нуля, что полностью предотвращает визуальное моргание маркеров WebGL и защищает
     *    активный открытый балун `mapboxgl.Popup` от внезапного закрытия при обновлении координат.
     */
    ...
    /**
     * РЕАКТИВНЫЕ ЭФФЕКТЫ СИНХРОНИЗАЦИИ КАМЕРЫ (FLYTO)
     *
     * Зачем нужен этот эффект:
     * 1. Односторонняя связь: Реализует паттерн «Единого источника правды». Карта самостоятельно
     *    управляет координатами экрана во время ручного скролла (через событие `idle`), но если
     *    пользователь совершает действие в UI (селект города или клик по карточке АЗС), экшены
     *    меняют сигналы `mapCenter`, `mapZoom` и взводят флаг-сигнал `isProgrammatic = true`.
     * 2. Изоляция анимации: Эффект реагирует на эти изменения и плавно переносит камеру Mapbox
     *    с помощью метода `.flyTo()`.
     * 3. Защита от зацикливания: Строка `if (programmatic)` и последующий синхронный сброс флага
     *    в `false` намертво блокируют обратную реактивную связь. Карта «летит» к цели, но последующая
     *    генерация координат в событии `idle` больше не заставляет эффект дергать камеру повторно,
     *    полностью предотвращая бесконечные циклы анимаций.
     */
  ...
  public destroyMap = () => {
    ...
    this.geojsonFeaturesCache.clear()
  }
  ...
  private handleMapIdle = () => {
    ...
    this.debounceTimer = setTimeout(() => {
      ...
      if (this.bbox.value !== newBboxString) { this.bbox.value = newBboxString }
  ...
  private renderMarkers(stations: Station[] | null) {
    ...
    if (stations && Array.isArray(stations)) {
      // НАКОПЛЕНИЕ: Добавляем только уникальные фичи в Map-кэш
      stations.filter(station => station.lat && station.lng).forEach(station => { if (!this.geojsonFeaturesCache.has(station.id)) { this.geojsonFeaturesCache.set(station.id, { type: 'Feature' as const, geometry: { type: 'Point' as const, coordinates: [station.lng, station.lat] }, properties: { id: station.id, station: JSON.stringify(station) } }) } })
    }

    // Рендерим склеенный массив из накопительного кэша
    source.setData({ type: 'FeatureCollection', features: Array.from(this.geojsonFeaturesCache.values()) })
