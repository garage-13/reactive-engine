...
Метод `engine.use` — это более ранняя встроенная альтернатива этим хукам. Он делает абсолютно то же самое, но требует предварительного вызова `engine.setReactAdapters(useState, useEffect)`.Вы можете оставить `engine.use` внутри ядра для тех разработчиков, которые любят вызывать методы прямо из инстанса движка, либо полностью заменить его в документации на созданный нами независимый хук `useReactiveValue`, так как он работает быстрее и нативнее для React 18+.
